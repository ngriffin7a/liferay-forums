 ...
 *   - No emojis in out.println (parser chokes).
 *   - No outer try/catch wrapping a large block (parser chokes).
 *     Use plain OK / FAIL / WARN prefixes for status lines.
 */
...
// nameToPortletResourceName - used for ADD_OBJECT_ENTRY (portlet resource)
...
FORUM_OBJECT_NAMES.each { name ->
    ...
    if (resourceName) {
        ...
        out.println("  WARN  ${name} - not found")
    }
}
...
if (discoveredCount == 0) {
    out.println("FAIL No Forum objects found. Skipping company ${companyId}.")
    out.println("")
    return
}
...
def grantCompanyPermission = { String roleName, String resourceName, String actionId ->
    try {
        ...
        out.println("  OK  ${roleName} -> ${actionId} (company scope)")
        granted++
    }
    catch (com.liferay.portal.kernel.exception.NoSuchRoleException e) {
        out.println("  FAIL  Role '${roleName}' not found")
        failed++
    }
    catch (Exception e) {
        out.println("  FAIL  ${roleName} -> ${actionId} - ${e.class.simpleName}: ${e.message}")
        failed++
    }
}
...
FORUM_OBJECT_NAMES.each { name ->
    ...
    if (!resourceName) {
        out.println("WARN  Skipping ${name} - no ObjectDefinition found")
        skipped++
        return
    }
    ...
    //
    // Do NOT grant UPDATE or DELETE to Site Member at company scope. When an
    // Object entry is created, Liferay's ObjectEntryLocalService calls
    // ResourceLocalService.addModelResources(), which grants the creator
    // individual-scope UPDATE and DELETE on their own entry. A company-scope
    // grant here would let any Site Member modify or delete entries they do
    // not own.
...
if (existingEntry) {
    ...
    if (!existingEntry.isDefaultSAPEntry() || !existingEntry.isEnabled()) {
        ...
        out.println("  OK Updated - set Default=true, Enabled=true")
    } else {
        out.println("  OK Already configured correctly")
    }
} else {
    ...
    out.println("  OK Created SAP '${SAP_NAME}'")
